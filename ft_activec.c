...
int		ft_activeflagc(char **fl, char **tab, char **str)
{
	int len;
	int j;
	int i;
	...
	if (*fl && **fl != '\0' && (j = 0) >= 0)
	{
		...
		{
			if (*str == NULL && (j = 0) >= 0)
				(*str) = malloc(ft_strlen((*fl)) + 1);
			if (ft_isdigit((*fl)[i]))
				(*str)[j++] = (*fl)[i];
			if ((*str) && ft_cnd2((*fl)[i], (*str)[j - 1], i, *fl)
				> 0 && ((*str)[j] = '\0') == '\0')
				if (!(len += ft_toactivec(tab, fl, str, i)))
					break ;
			if (*str && ft_cnd2s((*fl)[i], (*str)[j - 1], i, &(*fl)[0]) < 0)
				ft_free1(str);
		}
	}
	else
